...
export const horizontalParallaxEffect: ScrollEffect = {
    ...
    create(element: HTMLElement): EffectInstance {
        const movementDistance = parseAttr(element, 'data-parallax-movement', DEFAULT_MOVEMENT_DISTANCE);
        const minimumOpacity = parseAttr(element, 'data-parallax-min-opacity', DEFAULT_MIN_OPACITY);
        const freezePointRatio = parseAttr(element, 'data-parallax-freeze-point', DEFAULT_FREEZE_POINT);
        ...
        return {
            ...
            update(ctx: ScrollContext) {
                ...
                let progress = Math.max(0, (Math.abs(distanceFromCenter) - freezePixels) / (ctx.viewportHeight / 2));
                if (elementCenterY < viewportCenterY) {
                    progress = Math.max(0, (-distanceFromCenter - freezePixels) / (ctx.viewportHeight / 2));
                }
                ...
                element.style.opacity = String(minimumOpacity + (1 - minimumOpacity) * (1 - opacityProgress));
            },
        };
    },
};
