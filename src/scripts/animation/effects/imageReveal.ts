...
export const imageRevealEffect: ScrollEffect = {
    ...
    create(element: HTMLElement): EffectInstance | null {
        ...
        const freezePointRatioRaw = parseFloat(element.getAttribute('data-parallax-freeze-point') ?? '');
        ...
        return {
            ...
            update(ctx: ScrollContext) {
                ...
                const shadow1 = interpolateColor('rgba(0, 0, 0, 0.15)', 'rgba(0, 102, 255, 0.3)', progress);
                const shadow2 = interpolateColor('rgba(0, 0, 0, 0.0)', 'rgba(0, 102, 255, 0.2)', progress);
